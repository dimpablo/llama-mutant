...
show_menu() {
    ...
    echo -e "  ${GREEN}5)${NC} 🔧 Сборка из исходников"
    echo -e "  ${GREEN}6)${NC} 🔍 Анализ ограничений"
    echo -e "  ${GREEN}7)${NC} 🧪 Тестирование системы"
    echo -e "  ${GREEN}8)${NC} 📊 Статус системы"
    echo -e "  ${GREEN}9)${NC} 📚 Документация"
    echo -e "  ${GREEN}10)${NC} 🚪 Выход"
    echo ""
    echo -e "${NC}"
}
...
run_build_from_source() {
    echo -e "${CYAN}🔧 Запуск сборки llama.cpp из исходников...${NC}"
    echo -e "${YELLOW}💡 Это займет некоторое время${NC}"
    
    if [ -f "build_llama_from_source.sh" ]; then
        ./build_llama_from_source.sh
    else
        echo -e "${RED}❌ Скрипт build_llama_from_source.sh не найден${NC}"
        echo -e "${YELLOW}💡 Проверьте, что файл существует${NC}"
    fi
}

# Анализ ограничений
run_analyze_restrictions() {
    echo -e "${CYAN}🔍 Запуск анализа ограничений в исходниках...${NC}"
    echo -e "${YELLOW}💡 Анализ может занять несколько минут${NC}"
    
    if [ -f "analyze_restrictions.py" ]; then
        python3 analyze_restrictions.py
    else
        echo -e "${RED}❌ Скрипт analyze_restrictions.py не найден${NC}"
        echo -e "${YELLOW}💡 Проверьте, что файл существует${NC}"
    fi
}
...
show_documentation() {
    ...
    echo -e "  ${GREEN}🔧 README_BUILD_FROM_SOURCE.md${NC} - Сборка из исходников"
    echo ""
...
main() {
    ...
    while true; do
        ...
        echo -e "${YELLOW}Выберите режим (1-10):${NC} "
        read -r choice
